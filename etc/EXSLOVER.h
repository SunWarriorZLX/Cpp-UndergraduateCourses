...
public:
        ...
        for (int i = 0; s[i];)
        {
            ...
            */
            ...
            if (IsNums(s[i]))
            {
                ...
                reverse(temp.begin(), temp.end()); /* 反转，将高位放到栈顶，_size-1代表其位数 */
                for (; temp.size();)
                {
    ...
    出现问题的根本原因在于递归时运算符一定出栈；但在运算次序变化时，运算数不一定出栈 */
    {
        ...
        if (Operator.back() == '(') /* '('d的优先级最高，递归跳入'()'内的运算 */
        {
            Operator.pop_back();
            Processing();
            return;
        }
        if (Operator.back() == ')') /* '()'内运算结束，返回 */
        {
            Operator.pop_back();
            return;
        }
        ...
        double NumTemp;
        OperTemp = Operator.back();
        NumTemp = nums.back();
        Operator.pop_back();
        nums.pop_back();
        /* 当前运算级高于或等于下一运算级，进行当前的运算 */
        if (GetPriorityLevel(OperTemp) >= GetPriorityLevel(Operator.back()))
        {
